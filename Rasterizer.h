...
public:
  ...
  std::function<Vector3f(rasterizer *, const payload &)> get_texture =
      &rasterizer::direct_sample;
  ...
  void rasterizer_triangle(const triangle &tri,
                           const std::array<Vector4f, 3> &view_space_vertex,
                           std::vector<light> lights_to_rst);
  Vector3f fragment_shader(const payload &load,
                           std::vector<light> lights_to_rst);
